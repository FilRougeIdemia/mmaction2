...
'''def calculate_confusion_matrix(y_pred, y_real, normalize=None):
    ...
    if normalize not in ['true', 'pred', 'all', None]:
        raise ValueError("normalize must be one of {'true', 'pred', "
                         "'all', None}")
    if isinstance(y_pred, np.ndarray): # added
        y_pred = np.argmax(y_pred, axis=1)
    if isinstance(y_pred, list): # added
...
def confusion_matrix_sns(y_pred, y_real, normalize=None):
    ...
    cm = calculate_confusion_matrix(y_pred, y_real, normalize=None)
    confusion_mat = sns.heatmap(cm, annot=True, cmap='Blues', xticklabels=class_names, yticklabels=class_names)
    return confusion_mat

def precision_per_class(y_pred, y_real, normalize=None):
    cm = calculate_confusion_matrix(y_pred, y_real, normalize=None)
...
def recall_per_class(y_pred, y_real, normalize=None):
    cm = calculate_confusion_matrix(y_pred, y_real, normalize=None)
    num_classes = cm.shape[0]
...
def f1_per_class(y_pred, y_real, normalize=None):
    cm = calculate_confusion_matrix(y_pred, y_real, normalize=None)
    num_classes = cm.shape[0]
    precision = precision_per_class(y_pred, y_real, normalize=None)
    recall = recall_per_class(y_pred, y_real, normalize=None)
    f1 = np.zeros(num_classes)

    for i in range(num_classes):
        f1[i] = 2 * precision[i] * recall[i] / (precision[i] + recall[i] + 1e-15)

    return f1

def wrong_predictions(y_pred, y_real):
    y_pred = np.argmax(y_pred, axis=1)
    wrong_predictions = [i for i in range(len(y_real)) if y_real[i] != y_pred[i]]
    return y_pred[wrong_predictions], y_real[wrong_predictions]'''
